...
import { getConfig, DISCOVERY_QUERIES, METRO_SUBLOCATIONS } from '../utils/config.js';
...
paginate = false // Whether to fetch all pages (up to 60 results)
) {
...
export async function discoverLeadsInMetro(metro, country, options = {}, onProgress) {
    ...
    try {
        for (const sublocation of sublocations) {
            ...
            for (const query of queries) {
                ...
                const places = await searchPlaces(query, { lat: sublocation.lat, lng: sublocation.lng }, radius, paginate);
...
export async function discoverLeadsInMultipleMetros(metros, country, options = {}, onProgress) {
    const totalResult = {
        leadsFound: 0,
        leadsNew: 0,
        leadsDuplicate: 0,
